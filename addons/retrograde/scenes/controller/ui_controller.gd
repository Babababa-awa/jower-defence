...
func hide_ui(alias_: StringName) -> void:
	...
	if ui != null:
		ui.hide_ui()
	elif alias_ == &"hud" and Core.hud != null:
		Core.hud.visible = false
...
func prepare_ui_alias(alias_: StringName, from_alias_: StringName) -> StringName:
	if alias_ != &"parent":
		if alias_ == &"difficulty" and not Core.ENABLE_GAME_DIFFICULTY:
			alias_ = &"level_select"
	
		if alias_ == &"level_select" and not Core.ENABLE_LEVEL_SELECT:
			alias_ = &"start"
	...
	if from_alias_ == &"level_select" and Core.ENABLE_GAME_DIFFICULTY:
		return &"difficulty"
